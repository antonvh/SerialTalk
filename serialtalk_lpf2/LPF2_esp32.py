...
class LPF2(object):
    #------ callback command
     ...
     def __init__(self, uartChannel, txPin, rxPin, modes = defaultModes, baud=115200, sensor_type = WeDo_Ultrasonic, timer = 4, freq = 5):
          self.txPin = txPin
          self.rxPin = rxPin
...
# -------- Payload definition
     ...
     def load_maxarray(self, array):   # array of 16 bytes
          bit = 4
          payload = bytearray([CMD_Data | (bit << CMD_LLL_SHIFT) | self.current_mode])+array
          self.payload = self.addChksm(payload)
...
# -----  Init and close
     ...
     def init(self):
          ...
          utime.sleep_ms(500)
          self.tx.value(1)
          self.uart=machine.UART(2,baudrate=self.baud,rx=self.rxPin,tx=self.txPin)
          self.writeIt(b'\x00')
...
# -----   Start everything up
     ...
     def initialize(self):
          ...
          self.writeIt(self.defineBaud(self.baud))
